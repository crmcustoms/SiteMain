...
export async function generateStaticParams() {
  try {
    ...
    return i18n.locales.flatMap(locale => {
      ...
      const localParams = getAllContent("blog", locale).map(entry => ({ slug: entry.slug, lang: locale }));
...
export async function generateMetadata({
  ...
  // Статьи, опубликованные локально через markdown (content/blog/uk/*.md), проверяем первыми
  const localArticle = getContentBySlug("blog", safeLocale, slug);
  if (localArticle) {
    return {
      title: localArticle.title,
      description: localArticle.excerpt,
      alternates: { canonical: `https://crmcustoms.com/${safeLocale}/blog/${localArticle.slug}` },
      openGraph: {
        title: localArticle.title,
        description: localArticle.excerpt,
        type: "article",
  ...
  if (localArticle) {
    return (
      <ArticleRenderer
        ...
        url={`https://crmcustoms.com/${safeLocale}/blog/${localArticle.slug}`}
